...
// Set the file name using the config path and state number
  ...
  if(istart_typ_cp>=3){
    sprintf(fname, "%s.s%d.k%d.b%d.t%d/vState%d.out",config.dataPath,ispin_ind,kpoint_ind,ibead_ind,itemper_ind,
                                                     ind_state+1);
...
// If you've got the whole state, write it out and then invoke the reduction.
  ...
  if(countFileOut==nchareG){
     ...
     sprintf(psiName, "%s.s%d.k%d.b%d.t%d/state%d.out",config.dataPathOut,
                                  ispin_ind,kpoint_ind,ibead_ind,itemper_ind,ind_state);
     sprintf(vpsiName,"%s.s%d.k%d.b%d.t%d/vState%d.out",config.dataPathOut,
                                  ispin_ind,kpoint_ind,ibead_ind,itemper_ind,ind_state);
