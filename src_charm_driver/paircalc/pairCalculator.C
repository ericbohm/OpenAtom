...
void createPairCalculator(bool sym, int s, int grainSize, int numZ, int* z, 
			  ...
			  int priority, CkVec <CkGroupID> mCastGrpId, int numChunks, int orthoGrainSize, int useEtoM, bool collectTiles, bool streamBWout, bool delayBWSend) {
  ...
  if(!mapid) {
    ...
    pairCalculatorProxy = CProxy_PairCalculator::ckNew(sym, grainSize, s, numChunks,  cb, cb_aid, cb_ep, cb_ep_tol, conserveMemory, lbpaircalc, cpreduce, orthoGrainSize, collectTiles, streamBWout, delayBWSend, options);
  }
  ...
  if(sym)
	...
	for (int s2 = s1; s2 < s; s2 += grainSize) {
	  for (int c = 0; c < numChunks; c++) {
		...
		insert(sym, grainSize, s, numChunks,  cb, cb_aid, cb_ep, cb_ep_tol, conserveMemory, lbpaircalc, cpreduce, orthoGrainSize, collectTiles, streamBWout, delayBWSend );
	    }
	    else
	      {
#ifdef _PAIRCALC_CREATE_DEBUG_
...
#endif
		pairCalculatorProxy(z[numX],s1,s2,c).
		  insert(sym, grainSize, s, numChunks, cb, cb_aid, cb_ep, cb_ep_tol, conserveMemory, lbpaircalc, cpreduce, orthoGrainSize, collectTiles, streamBWout, delayBWSend, proc);
		proc++;
