...
#define DEBUG_MESSAGEDATACOLLATOR_EACHARRIVAL
	...
	/// else, if this is a fresh batch of messages
	else 
	  {
	    ...
	    if (dataBuffer!=NULL && isBufferMine && reqdBufferSizeInUnits > bufferSizeInUnits)
	      {
...
#ifdef TEST_ALIGN
		CkAssert(((unsigned int)dataBuffer)%16==0);
#endif
...
#ifdef TEST_ALIGN
	CkAssert(((unsigned int) msg->data())%16==0);
	CkAssert(((unsigned int) &(dataBuffer[offset*numUnitsInMsg]))%16==0);
#endif
