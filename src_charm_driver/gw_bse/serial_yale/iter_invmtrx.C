...
void iter_invmtrx(CMATRIX* eps, USRINPUT usrin, int N){
    ...
    for(int i=0; i<N; i++){
        for(int j=0; j<N; j++){
	  eps->get(i,j) = X[i+j*N];   // X is in column mojor after myGEMM, so we change it to row major here
        }
    }//end step 3


#ifdef DEBUG
    printf("printing out epsilon inverse matrix\n");
    for(int r=0; r<3; r++){
      for(int c=0; c<3; c++){
        printf(" epsilon inverse at g=%d g'=%d  %lg  %lg  \n",r,c,eps->get(r,c).re,eps->get(r,c).im);
      }
    }
#endif
