...
CkArrayID Builder::build(cp::paircalc::InstanceIDs &asymmHandle, PeListFactory getPeList)
{
    ...
    int size = (cfg.numStates/cfg.grainSize)*(cfg.numStates/cfg.grainSize);

    if(size >= CmiNumPhysicalNodes()){
    	CProxy_NodeMapOrthoArray orthoCrayMap = CProxy_NodeMapOrthoArray::ckNew(cfg.numStates,cfg.grainSize,cores_per_node,0,CmiNumPhysicalNodes(),0);
