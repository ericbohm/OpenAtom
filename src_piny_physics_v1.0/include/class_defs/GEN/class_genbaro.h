...
class GENBARO {
  ...
  public:
    ...
    GENBARO(){
      iopt = 0;
      len_nhc = 0;
      hmato = (double *) cmalloc(9*sizeof(int),"constructor genbaro")-1;
    }
    ~GENBARO(){};
    ...
    void state_class_out(){
      ...
      int i;
      char fileName [255];
      sprintf (fileName, "%d_genbaro.state", CkMyPe());
      ...
      // dbl  arrays
      if(iopt>0){
        for(i=1;i<=len_nhc;i++){fprintf(fp,"mass_vol_nhc[%d] %g",mass_vol_nhc[i]);}
        for(i=1;i<=len_nhc;i++){fprintf(fp,"gkt_vol[%d] %g",gkt_vol[i]);}
      }
      fclose(fp);
    } // end routine
