...
typedef struct CP_PARSE {
  ...
  int kpt_file_name_set;        /* Num : kpt file set by user*/
  ...
  char *kpt_file_name;         /* char: Kpoints file name: has set flag*/
...
typedef struct FILENAME_PARSE{
  ...
  NAME def_lj_name;            /* Chr: User force field pot lib file  */
  NAME user_lj_name;           /* Chr: Default force field pot lib file*/
...
typedef struct CVPS  { NAME atm1;CVPS(){}; ~CVPS(){};}CVPS;           /* Chr: atms in VPS type */
typedef struct CLJ   { NAME atm1;CLJ(){}; ~CLJ(){};}CLJ;             /* Chr: atms in FF  type */
typedef struct CBOND { NAME atm1,atm2,label;CBOND(){};~CBOND(){};}CBOND;/* Chr: atms in bond type*/
typedef struct CBEND { NAME atm1,atm2,atm3,label;CBEND(){};~CBEND(){};}CBEND; /* Chr: atms in bend type*/
typedef struct CTORS { NAME atm1,atm2,atm3,atm4,label;CTORS(){};~CTORS(){};}CTORS;  
typedef struct CATM_LAB { NAME atm1,atm2,atm3,atm4,label;CATM_LAB(){};~CATM_LAB(){};}CATM_LAB;  
typedef struct CGRP_CONS{ NAME atm1,atm2,atm3,atm4,label[7];CGRP_CONS(){};~CGRP_CONS(){};}CGRP_CONS;  
...
typedef struct DATA_BASE_ENTRIES{double eq,eq_res,eqb;
...
typedef struct WILD { int atm1,atm2,atm3,atm4,label,good;WILD(){};~WILD(){};}WILD;
