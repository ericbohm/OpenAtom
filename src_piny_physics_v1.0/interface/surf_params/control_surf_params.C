...
void set_surf_splin(double sig[],double eps[], int surf_label[],
    MDSURFACE *surface, int natm_typ)
...
{ /* begin routine */
  ...
  z   = (double *)cmalloc(nsplin*sizeof(double),"spline_surf")-1;
  sw  = (double *)cmalloc(nsplin*sizeof(double),"spline_surf")-1;
  dsw = (double *)cmalloc(nsplin*sizeof(double),"spline_surf")-1;
  ...
  cfree(&z[1],"spline_surf");
  cfree(&sw[1],"spline_surf");
  cfree(&dsw[1],"spline_surf");
