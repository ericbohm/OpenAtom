...
/* 0) Allocate temporary arrays                                         */
  ...
  num_typ = (int *) cmalloc(natm_typ*sizeof(int),"get_clong")-1;
  index   = (int *) cmalloc(natm_typ*sizeof(int),"get_clong")-1;
  clongm  = (double *) cmalloc(ninter*sizeof(double),"get_clong")-1;
...
/* III) Free temporaries                                                */
  ...
  cfree(&num_typ[1],"get_clong");
  cfree(&index[1],"get_clong");
  cfree(&clongm[1],"get_clong");
...
void get_clong_lj_explicit(int natm_tot,int natm_typ,
                           int iatm_typ[],
                           double epsilon[],double sigma[],
                           double cutoff[],double cutoff_res[],
	                   double *clong,double *clong_res,
                           int iswit, double rheal)
...
// II) Get the long range correction                                     
  ...
  double cutoff_ij,cutoff_res_ij,epsilon_ij,sigma_ij,c6m;
  ...
  *clong_res= 0.0;
  ...
  if(iswit==0){
    ...
    for(j=i;j<= natm_typ;j++){
      cutoff_ij = (cutoff[i]+cutoff[j])/2.0;
      cutoff_res_ij = (cutoff_res[i]+cutoff_res[j])/2.0;
      epsilon_ij    = sqrt(epsilon[i]*epsilon[j]);
      sigma_ij      = (sigma[i]+sigma[j])/2.0;
      c6m  = 4.0*epsilon_ij*(sigma_ij*sigma_ij*sigma_ij*sigma_ij*sigma_ij*sigma_ij);

      if(j==i){
        clongm = (num_typ[i]*(num_typ[i]-1))/2;
      }else{
        clongm = (num_typ[i]*num_typ[j]);
      }

      rmin = (cutoff_ij < cutoff_res_ij ? cutoff_ij : cutoff_res_ij);

     *clong -=  fpi3*c6m*clongm/pow(cutoff_ij,3.0);

     *clong_res -= fpi*c6m*clongm*(
                   (1.0/(rheal*rheal))*(1.0/rmin+1.0/(rmin-rheal))
                  +(2.0/(rheal*rheal*rheal))*log((rmin-rheal)/rmin));
    }//endfor
    }//endfor
    ...
    for(j=i;j<=natm_typ;j++){
     ...
     if((c6m > 0)&&(cutoff_ij - rheal>0)){
      rmin        = (cutoff_ij < cutoff_res_ij ?  cutoff_ij : cutoff_res_ij);
      fact        = (1.0/(rheal*rheal))*(1.0/cutoff_ij + 1.0 / (cutoff_ij-rheal))
                  + (2.0/(rheal*rheal*rheal))*log((cutoff_ij - rheal)/cutoff_ij);
      *clong     -= (fpi*c6m*clongm*fact);
      fact_res    = (1.0/(rheal*rheal))*(1.0/rmin+1.0/(rmin-rheal))
                   +(2.0/(rheal*rheal*rheal))*log((rmin-rheal)/rmin);
      *clong_res -= fpi*c6m*clongm*fact_res;
     }//endif
