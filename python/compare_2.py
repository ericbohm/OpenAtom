def compare_2(testoutput, reffile, magnum):
	import compare
	import stripNumbers
		...
		if testcontent[0].find('Psi[',0,len(testcontent[0])-1) < 0:
			...
			while 2 > 1:
				...
				if refcontent[counter].find(keyword,0,len(refcontent[counter])-1) >= 0:
					...
					if compare.compare(testlist[1], reflist[0], magnum) == False:
						return False
		...
		if testcontent[0].find('Psi[',0,len(testcontent[0])-1) >= 0:
			...
			while 2 > 1:
				if counter == reflistlen or counter == testlistlen:
					...
					del testcontent[0]
					break
				if compare.compare(testlist[counter], reflist[counter], magnum) == False:
					return False
				counter = counter + 1
